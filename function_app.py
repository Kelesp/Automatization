...
# Carpeta que debe contener ya dos archivos de texto:
#   politicas_txt/politica_cuidado.txt
#   politicas_txt/politica_envejecimiento.txt
...
# En Windows, ajusta la ruta a tu instalación de tesseract.exe si es necesario:
# pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
# En Linux (Ubuntu) suele estar en /usr/bin/tesseract, así que no hace falta tocarlo.
...
async def process_pdf(
    ...
    # 3.2) Guardar PDF de municipio en temporal
    try:
    ...
    # 3.3) Extraer texto del municipio (rango pedido)
    try:
    ...
    # ------------------------------------------------------------
    # 3.4) Llamada a OpenAI para párrafos relevantes (nuevo API)
    # ------------------------------------------------------------
    prompt_parrafos = f"""
...
"""
    try:
        # *** Aquí se llama a la nueva interfaz para chat completions ***
    ...
    except openai.OpenAIError as e_openai:  # capturamos la excepción con la nueva clase
        os.remove(tmp_path)
        raise HTTPException(status_code=500, detail=f"Error en llamada a OpenAI (párrafos): {e_openai}")

    # Limpiar delimitadores ``` si los hubiera
    ...
    try:
        if isinstance(resultados_parrafos, list) and resultados_parrafos:
            # Extraer cada “resumen” de los párrafos
            res_parciales = [
                item.get("resumen", "").strip()
                for item in resultados_parrafos
...
"""
    ...
    except openai.OpenAIError as e_resumen:
...
"""
    ...
    except openai.OpenAIError as e_cu:
...
"""
    ...
    except openai.OpenAIError as e_en:
...
# ------------------------------------------------------------
# 4) Arrancar Uvicorn directamente si ejecutas este script
# ------------------------------------------------------------
...
>>>>>>> 6d5f358f736c80c52728d01acea7e89c1554becd
